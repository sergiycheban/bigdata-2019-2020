...
function createListWallet() {
  ...
  let cryptocurrenciesObj = fs.readFileSync(
    "./storage/listofcryptocurrencies/cryptocurrencies.json"
  );
      ...
      if (obj.amount >= amout) {
        ...
        for (let index = 0; index < recipientInfo.length; index++) {
          if (recipientInfo[index].crypto == crypto) {
            ...
            isSend = true;
            fs.writeFileSync(fileRecipient, JSON.stringify(recipientInfo));
          }
        }
        if (!isSend) {
          recipientInfo.push({
